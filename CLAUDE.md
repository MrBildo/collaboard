...
- **CommentTools:** `add_comment` (comment text via `contentMarkdown` — canonical; `content` is a still-supported alias, `contentMarkdown` wins when both are passed; blocks archived cards), `update_comment` (edit comment text; own-or-admin-level; blocks archived cards; `contentMarkdown` canonical, `content` alias), `delete_comment` (blocks archived cards; own-or-admin-level)
