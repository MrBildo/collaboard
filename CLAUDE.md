...
| Card history | `GET /cards/{id}/history` — the card's description edit trail, newest-first, each revision attributed + timestamped. Query: `field` (default `description`, unknown → 400), `format` (`diff`\|`full`\|`both`, **REST default `both`**), `from`+`to` (both or neither; returns the single-object pair shape instead of the trail). Any authenticated user — visibility follows the card's. History accrues from a card's first description edit after #357 shipped (no back-fill); the trail's oldest revision carries the pre-existing value with **null** author/timestamp, because nobody observed it being written. Whole values are stored, diffs rendered on read (`UnifiedDiff`, git-style hunks, `\n` endings). Capture is staged by `CardHistoryHelper` from both the REST PATCH and MCP `update_card` paths so the two cannot drift. (#357) |
