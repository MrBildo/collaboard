...
- **CommentTools:** `add_comment` (comment text via `contentMarkdown` — required, sole param; blocks archived cards), `update_comment` (edit comment text; own-or-admin-level; blocks archived cards; `contentMarkdown` canonical, `content` alias), `delete_comment` (blocks archived cards; own-or-admin-level)
