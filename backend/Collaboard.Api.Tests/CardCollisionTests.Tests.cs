...
public class CardCollisionTests(CollaboardApiFactory factory) : IClassFixture<CollaboardApiFactory>, IDisposable
{
    ...
    public async Task McpUpdateCard_OnlyExpectedRevision_IsStillANoOp()
    {
        ...
        var (_, tools) = CreateTools();
        var result = await tools.UpdateCardAsync(CollaboardApiFactory.TestAdminAuthKey, cardId, expectedDescriptionRevision: 1);
