    ...
    return {
      ...
      setOpen: (value: boolean) => {
        ...
        if (propsRef.current.onOpenChange) {
          propsRef.current.onOpenChange(value, {} as never);
        }
  ...
  return (
    <ColorPickerContext.Provider value={contextValue}>
      <Popover
        defaultOpen={defaultOpen}
        open={open}
        onOpenChange={(open) => store.setOpen(open)}
        modal={modal}
      >
        <RootPrimitive {...rootProps} ref={composedRef} />
        {isFormControl && (
...
function ColorPickerTrigger(
  props: Omit<React.ComponentProps<typeof PopoverTrigger>, "className"> & { className?: string; disabled?: boolean },
) {
  const { disabled, className, ...triggerProps } = props;
